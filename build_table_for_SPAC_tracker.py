#Building CSV file that gets loaded into Google Sheets and provides data for a SPAC tracking table 
#Revision Tracker
# 20220206 Added SATL to New Space list
# 20220314 v1.0 of new script forked from build_CSV_for_Wix.py
...
#Only need new space companies in this case
...
with open('SPAC_table_data.csv', 'w') as output_file:
	with open('SPAC_bad_table_data.csv', 'w') as output_file_no_data:
		output = 'Company Name, Ticker, Yahoo Finance URL, Market Cap, Enterprise Value, Cash Per Share, Last Close\n'
		...
		for ticker in ticker_list:
			...
			company_ev = str(ticker_object.info['enterpriseValue'])
			company_cashpershare = str(ticker_object.info['totalCashPerShare'])
			company_lastclose = yf.download(ticker, today)['Close']
			company_lastclose = company_lastclose.iloc[0]
			company_lastclose = str(company_lastclose)
			...
			output = company_name + ',' + ticker + ',' + company_url + ',' + market_cap + ',' + company_ev + ',' + company_cashpershare + ',' + company_lastclose + '\n'
			#if (company_TTM == "None"):
				#do nothing
...
with open ('SPAC_table_data.csv', 'a', newline='') as f_object:
	writer_object = writer(f_object)
	writer_object.writerow(time_data)
	f_object.close

with open('SPAC_bad_table_data.csv', 'a') as f_object:
	writer_object = writer(f_object)
	writer_object.writerow(time_data)
	f_object.close
...
sheetName = 'Data_For_SPAC_Tracking_Table'
csvFile = 'SPAC_table_data.csv'
