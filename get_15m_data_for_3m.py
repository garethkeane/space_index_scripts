...
# 20220315 Chnage this one to get 15 minute snapshot for 3 months
...
daily_data = yf.download(ticker_list, period='60d', interval='15m')['Close']
reversed_daily_data = daily_data.iloc[::-1]
reversed_daily_data = reversed_daily_data[ticker_list]
reversed_daily_data.to_csv('test_60day_15minute_data.csv')
...
with open ('test_60day_15minute_data.csv', 'a', newline='') as f_object:
...
sheetName = 'Three_Month_Intraday_Data'
csvFile = 'test_60day_15minute_data.csv'

#sheet = client.open(spreadsheetId)
#sheet.clear(sheetName)
sheet = client.open(spreadsheetId)
active_sheet = sheet.worksheet('Three_Month_Intraday_Data')
active_sheet.clear()
